...
class MainWindow(QMainWindow):
    def __init__(self, engine):
        super().__init__()
        self.engine = engine
        self.setWindowTitle("Lohjinawi-64 | Mesin Putar Multijalur (Windows)")
        self.resize(1300, 800)

        self.setStyleSheet("""
            QMainWindow { background-color: #2b2d30; }
            QLabel { color: #dfdfdf; font-family: 'Segoe UI', Arial, sans-serif; }
        ...
        for i, dev in enumerate(devices):
            if dev['max_output_channels'] > 0:
                try:
                    ...
                    self.device_combo.addItem(f"{dev['name']} ({hostapi_name})", i)
                except: pass

        # Auto-Select ASIO if available
        for idx in range(self.device_combo.count()):
            if "ASIO" in self.device_combo.itemText(idx).upper():
                self.device_combo.setCurrentIndex(idx)
                break

        io_layout.addWidget(title_lbl)
        io_layout.addStretch()
        io_layout.addWidget(QLabel("Output Driver:"))
    ...
    def load_action(self):
        path, _ = QFileDialog.getOpenFileName(self, "Select Audio", "", "Audio Files (*.wav *.flac)")
        if path:
    ...
    def start_audio(self):
        try:
            ...
            QMessageBox.critical(self, "Hardware Error", f"Cannot open 64 channels on this device.\n\n{e}")
    ...
    def refresh_vis(self):
        if self.stream:
            ...
            if not self.is_scrubbing_ui and self.engine.total_frames > 0:
                ...
                self.time_label.setText(f"{int(self.engine.current_frame/48000)//60:02d}:{int(self.engine.current_frame/48000)%60:02d} / Progress")
